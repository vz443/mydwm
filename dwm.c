...
static int
isuniquegeom(Geom *unique, size_t n, Geom *info)
{
	while (n--)
...
updategeom(void)
{
...
#ifdef XINERAMA
	{
		...
		if (sr && sr->ncrtc > 0) {
			info = ecalloc(sr->ncrtc, sizeof(Geom));
			for (i = 0; i < sr->ncrtc; i++) {
				XRRCrtcInfo *ci = XRRGetCrtcInfo(dpy, sr, sr->crtcs[i]);
				if (ci && ci->width > 0 && ci->height > 0) {
					info[nn].x = ci->x;
					info[nn].y = ci->y;
					info[nn].width = ci->width;
					info[nn].height = ci->height;
					nn++;
				}
				if (ci)
					XRRFreeCrtcInfo(ci);
		...
		if (nn == 0) {
			...
			unique = ecalloc(nn, sizeof(Geom));
			for (i = 0, j = 0; i < nn; i++)
				if (isuniquegeom(unique, j, &info[i]))
					memcpy(&unique[j++], &info[i], sizeof(Geom));
			free(info);
			nn = j;
